...
```{r, warning = FALSE, message = FALSE, echo = TRUE}
library(tidyr)
# Step 1: Construct the osberved counts from teh raw data. This is beneficial because we'll pass
# this to ggplot to scale the points sizes.
...
## Step 2: Take the prediction data and join it to the observed counts so we have one plotting data frame.
# Then use that to generate the plot.
summarize_predictions(predictions) |>
  mutate(
    category = factor(category)
  ) |>
  ...
  ggplot(aes(x = prepost_label, y = mean_prob, color = category, group = category)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  geom_pointrange(aes(ymin = lower_ci, ymax = upper_ci, size = n_scaled),
                  position = position_dodge(width = 0.5),
                  linewidth = 1, alpha = 0.3) +
  # Change point offset for readability
  geom_line(position = position_dodge(width = 0.5)) + 
  facet_wrap(~vote_trump_label, ncol = 2) +
  labs(
    x = "",
    ...
    color = "Response\nCategory",
    size = "N",
    title = "Predicted Probabilities by Interview Period and Vote Choice",
    subtitle = "Points are mean probability with 95% confidence intervals"
  ) +
  theme_minimal() +
  theme(
    legend.position = "right",
  ...
  # Color modifications
  scale_color_manual(values = c("1" = "#E41A1C", "2" = "#377EB8", "3" = "#4DAF4A", "4" = "#984EA3", "5"= "pink"),
