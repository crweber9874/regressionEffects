...
```{r, warning = FALSE, message = FALSE, echo = TRUE}
### Estimate
## Truncate the data to only the strong agree category 
...
attend_march = polr(as.factor(attend_march) ~ prepost*vote_trump , 
               data = recode_data)
...
I construct the marginal effect as follows:
...
#' Calculate Marginal Effects from a brms Model
#' Contrast variable == the variable for the marginal effect, `prepost`
#' Specify contrast values, default is 0 and 1, but for continuous variables, use quantile
#' Moderators == list of other variables to hold constant
...
calculate_marginal_effects <- function(model, 
  ...
  # 1. Construct the grid, based on variable of marginal effect, DMatrix from above
  d_grid <- expand_grid(
    {{ contrast_var }} := contrast_vals,
    moderators
  )
  
  # 2. Get Draws and calculate prosterior predictions, draws passing dmatrix, see above
...
predictions_march <- predict_ordinal_probs(
  ...
  model = attend_march,
  n_draws = 1000) |>
  mutate(variable = "rally")
predictions_criticize <- predict_ordinal_probs(
  design_matrix = design_matrix,
  model = criticize_election,
  n_draws = 1000) |>
  mutate(variable = "criticize")
predictions_burn <- predict_ordinal_probs(
  design_matrix = design_matrix,
  model = burn_flag,
  n_draws = 1000) |>
  mutate(variable = "burn_flag")
predictions_court <- predict_ordinal_probs(
  design_matrix = design_matrix,
  model = court,
  n_draws = 1000) |>
  mutate(variable = "court")
predictions_recount <- predict_ordinal_probs(
  design_matrix = design_matrix,
  model = recount,
  n_draws = 1000) |>
  mutate(variable = "recount")

rbind(predictions_march,
      predictions_criticize,
      predictions_burn,
      predictions_court,
      predictions_recount) -> predictions_all
...
```{r, cache = FALSE}
plot_dat <- prepare_prediction_data(
  recode_data, 
  predictions_all ,
  category = "prepost",
...
Each sunflower plot represents the relative size in the observed counts (so if 1=agree; 0 = otherwiese), the idea being it shows the relative prevalence in the sample data. Then each point is connected to the center of the sunflower ll. this just a line plot with nodes scaled by prevalence.
...
# Combine into single file maintaining structure
plot_data_all <- list(
  plot_dat,
  observed_counts = bind_rows(
    plot_data_march$observed_counts |> mutate(outcome = "attend_march"),
    plot_data_burn$observed_counts |> mutate(outcome = "burn_flag"),
    plot_data_court$observed_counts |> mutate(outcome = "court"),
    plot_data_criticize$observed_counts |> mutate(outcome = "criticize_election"),
    plot_data_recount$observed_counts |> mutate(outcome = "recount")
  )
)

plot_data_all$plotting_data |> filter(outcome == "burn_flag")

```

```{r, cache = FALSE}

plot_dat <- prepare_prediction_data(recode_data, predictions_burn,  # Use recode_data, not data
                                    category_var = "burn_flag",
